...
XDB_PRIVATE(int)
_ipv4_sub_compare(const bytes_ip_t *ip_bytes, int bytes, const char *buffer,
                  int offset) {
  register int i0, i1;
...
static xdb_version_t _ip_version_list[] = {
    ...
    // 38 = 16 + 16 + 2 + 4
    {xdb_ipv6_id, "IPv6", xdb_ipv6_bytes, xdb_v6_index_size,
     xdb_ip_sub_compare},

    // END
    {0, NULL, 0, 0, NULL}};

XDB_PUBLIC(xdb_version_t *) xdb_version_v4() { return &_ip_version_list[0]; }

XDB_PUBLIC(xdb_version_t *) xdb_version_v6() { return &_ip_version_list[1]; }
...
XDB_PUBLIC(xdb_version_t *)
xdb_parse_ip(const string_ip_t *ip_string, bytes_ip_t *buffer, size_t length) {
...
XDB_PUBLIC(xdb_version_t *)
xdb_parse_v4_ip(const string_ip_t *ip_string, bytes_ip_t *buffer,
                size_t length) {
  struct in_addr addr;
...
XDB_PUBLIC(xdb_version_t *)
xdb_parse_v6_ip(const string_ip_t *ip_string, bytes_ip_t *buffer,
                size_t length) {
...
XDB_PUBLIC(int)
xdb_ip_to_string(const bytes_ip_t *ip_bytes, int bytes, char *ip_string,
                 size_t length) {
...
XDB_PUBLIC(int)
xdb_v4_ip_to_string(const bytes_ip_t *ip_bytes, char *ip_string,
                    size_t length) {
...
XDB_PUBLIC(int)
xdb_v6_ip_to_string(const bytes_ip_t *ip_bytes, char *ip_string,
                    size_t length) {
  if (!ip_bytes || !ip_string || length == 0) {
    return -1;
  }
...
XDB_PUBLIC(int)
xdb_ip_sub_compare(const bytes_ip_t *ip1, int bytes, const char *buffer,
                   int offset) {
